...
#### H4. AI 请求取消机制已实现，但可恢复体验不足
- **文件**: `frontend/src/app/page.tsx`，`frontend/src/lib/stream.ts`
- **问题描述**: 已支持 `AbortSignal` + 取消按钮，但取消后的状态反馈、阶段回执与一键重试流程仍可优化
- **影响**: 用户取消后仍不清楚“请求停在何阶段”“下一步该做什么”
- **建议**: 增加阶段化状态机和取消后推荐动作（重试/重拍/改文本）
